...
import { resourceLoaders } from "./resources.generated";
...
i18nInstance
  ...
  .use(
    resourcesToBackend(async (language: string, namespace: string) => {
      const loader = resourceLoaders[`${language}/${namespace}`];
      if (!loader) throw new Error(`No locale bundle for ${language}/${namespace}`);
