...
class TestMagicLinkGenerate:
    ...
    def setup_user(self, db):
        ...
        user = User.objects.create(email="user@pm.cenz.pro")
    ...
    def test_magic_generate(self, mock_magic_link, api_client, setup_user, setup_instance):
        ...
        ri = redis_instance()
        ri.delete("magic_user@pm.cenz.pro")

        response = api_client.post(url, {"email": "user@pm.cenz.pro"}, format="json")
        ...
        assert args[0] == "user@pm.cenz.pro"  # First arg should be the email
    ...
    def test_max_generate_attempt(self, mock_magic_link, api_client, setup_user, setup_instance):
        ...
        for _ in range(4):
            api_client.post(url, {"email": "user@pm.cenz.pro"}, format="json")
...
class TestSignInEndpoint:
    ...
    def test_password_validity(self, django_client, setup_user, setup_instance):
        ...
        url = reverse("sign-in")
        response = django_client.post(url, {"email": "user@pm.cenz.pro", "password": "user123"}, follow=True)
    ...
    def test_user_login(self, django_client, setup_user, setup_instance):
        ...
        response = django_client.post(url, {"email": "user@pm.cenz.pro", "password": "user@123"}, follow=False)
    ...
    def test_next_path_redirection(self, django_client, setup_user, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "user@pm.cenz.pro", "password": "user@123", "next_path": next_path},
            follow=False,
        )
...
class TestMagicSignIn:
    ...
    def test_expired_invalid_magic_link(self, django_client, setup_user, setup_instance):
        ...
        url = reverse("magic-sign-in")
        response = django_client.post(url, {"email": "user@pm.cenz.pro", "code": "xxxx-xxxxx-xxxx"}, follow=False)
    ...
    def test_user_does_not_exist(self, django_client, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "nonexistent@pm.cenz.pro", "code": "xxxx-xxxxx-xxxx"},
            follow=True,
        )
    ...
    def test_magic_code_sign_in(self, mock_magic_link, django_client, api_client, setup_user, setup_instance):
        ...
        response = api_client.post(gen_url, {"email": "user@pm.cenz.pro"}, format="json")
        ...
        ri = redis_instance()
        user_data = json.loads(ri.get("magic_user@pm.cenz.pro"))
        token = user_data["token"]

        # Use Django client to test the redirect flow without following redirects
        url = reverse("magic-sign-in")
        response = django_client.post(url, {"email": "user@pm.cenz.pro", "code": token}, follow=False)
    ...
    def test_magic_sign_in_with_next_path(self, mock_magic_link, django_client, api_client, setup_user, setup_instance):
        ...
        response = django_client.post(
            url,
            {"email": "user@pm.cenz.pro", "code": token, "next_path": next_path},
            follow=False,
        )
...
class TestMagicSignUp:
    ...
    def test_user_already_exists(self, django_client, db, setup_instance):
        ...
        User.objects.create(email="existing@pm.cenz.pro")

        url = reverse("magic-sign-up")
        response = django_client.post(url, {"email": "existing@pm.cenz.pro", "code": "xxxx-xxxxx-xxxx"}, follow=True)
    ...
    def test_expired_invalid_magic_link(self, django_client, setup_instance):
        ...
        url = reverse("magic-sign-up")
        response = django_client.post(url, {"email": "new@pm.cenz.pro", "code": "xxxx-xxxxx-xxxx"}, follow=False)
