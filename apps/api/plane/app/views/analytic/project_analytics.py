...
class ProjectAdvanceAnalyticsStatsEndpoint(ProjectAdvanceAnalyticsBaseView):
    ...
    def get_work_items_stats(self, project_id, cycle_id=None, module_id=None) -> Dict[str, Dict[str, int]]:
        ...
        if cycle_id is not None:
            ...
            base_queryset = Issue.issue_objects.filter(**self.filters["base_filters"], project_id=project_id)
        return (
            # Traverse the IssueAssignee join and keep only non-soft-deleted links so assignees that
            # were removed from a work item are no longer counted (the m2m `assignees` traversal would
            # otherwise still include them).
            base_queryset.filter(issue_assignee__deleted_at__isnull=True)
            .annotate(display_name=F("issue_assignee__assignee__display_name"))
            .annotate(assignee_id=F("issue_assignee__assignee_id"))
            .annotate(avatar=F("issue_assignee__assignee__avatar"))
            .annotate(
                avatar_url=Case(
                    # If `avatar_asset` exists, use it to generate the asset URL
                    When(
                        issue_assignee__assignee__avatar_asset__isnull=False,
                        then=Concat(
                            Value("/api/assets/v2/static/"),
                            "issue_assignee__assignee__avatar_asset",
                            Value("/"),
                        ),
                    ),
                    # If `avatar_asset` is None, fall back to using `avatar` field directly
                    When(
                        issue_assignee__assignee__avatar_asset__isnull=True,
                        then="issue_assignee__assignee__avatar",
                    ),
                    default=Value(None),
